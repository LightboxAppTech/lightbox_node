...
module.exports = {
  ...
  requestToJoin: async (req, res) => {
    try {
      ...
      if (alreadyInList >= 0 || alreadyMember >= 0) return res.status(400).end()
    ...
    try {
      ...
      for (let i = 0; i < projects.length; i++) {
        for (let j = 0; j < projects[i].project_requests.length; j++) {
          let { fname, lname, _id, semester, title, thumbnail_pic } =
    ...
    try {
      ...
      project
        ...
        .then((data) => {
          return res
            .status(200)
            .json({ message: 'Project Completed Successfully' })
        })
        .catch((e) => {
          console.error(e)
          return res.status(500).json({ message: 'Something Went Wrong' })
